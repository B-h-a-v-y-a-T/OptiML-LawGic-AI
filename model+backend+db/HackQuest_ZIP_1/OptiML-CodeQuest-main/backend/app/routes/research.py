...
async def research(
    ...
    return {"input_text": combined_text, "prediction": analysis_payload, "document_id": saved_id}
from fastapi import APIRouter, UploadFile, File, Form, Depends
from typing import Optional
from sqlalchemy.ext.asyncio import AsyncSession
from app.utils.model_loader import predict
...
async def research(
    ...
    # Use the same model; if model was built for contract analysis, it still returns JSON; acceptable as placeholder
    prediction_result = predict(combined_text)
